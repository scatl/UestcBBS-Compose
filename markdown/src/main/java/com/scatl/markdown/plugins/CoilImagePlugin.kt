...
/**
 * Created by tanlei02 at 2024/12/9 20:11:10
 */
class CoilImagePlugin(
    coilStore: CoilStore,
    imageLoader: ImageLoader
): AbstractMarkwonPlugin() {

    private var coilAsyncDrawableLoader = CoilAsyncDrawableLoader(coilStore, imageLoader)
    ...
    companion object {
        fun create(context: Context): CoilImagePlugin {
        ...
        fun create(context: Context, imageLoader: ImageLoader): CoilImagePlugin {
            return create(object : CoilStore {
                override fun load(drawable: AsyncDrawable) =
                    ImageRequest
                        .Builder(context)
        ...
        fun create(coilStore: CoilStore, imageLoader: ImageLoader) = CoilImagePlugin(coilStore, imageLoader)
    }
...
//                        DrawableUtils.applyIntrinsicBoundsIfEmpty(result)
                        if (result.bounds.isEmpty) {
                            result.bounds = Rect(0, 0, 2000, (result.intrinsicHeight * (2000f / result.intrinsicWidth.toFloat())).toInt())
                        }
